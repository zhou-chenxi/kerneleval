...
class Gaussian:
	...
	"""
	This is a class of evaluating the Gaussian kernel function.
	
	Attributes
	----------
	data : numpy.ndarray
		The array at which the Gaussian kernel function is to evaluated.
	N, d : int, int
	...
	kernel_matrix(new_data)
		Evaluates the Gaussian kernel function at new_data.
	
	kernel_single_eval(new_data)
		Evaluates the Gaussian kernel function at new_data involving double `for` loops;
	...
	def kernel_matrix(self, new_data):
		...
		"""
		Evaluates the Gaussian kernel function at new_data.
		Each entry is k(X_i, Y_j),
		where X_i corresponds to the i-th row of data,
		...
		-------
		numpy.ndarray
			The array of Gaussian kernel function evaluations.
	...
	def kernel_single_eval(self, new_data):
		...
		for i in range(self.N):
			for j in range(n):
				output[i][j] = np.exp(- np.sum((self.data[i] - new_data[j]) ** 2) / (2 * self.bw ** 2))
