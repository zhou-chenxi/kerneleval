...
class RationalQuadratic:
	...
	"""
	This is a class of evaluating the rational quadratic kernel function.

	Attributes
	----------
	data : numpy.ndarray
		The array at which the rational quadratic kernel function is to evaluated.
	N, d : int, int
		The shape of data array.
	bw : float
		The bandwidth parameter in the rational quadratic kernel function; must be strictly positive.

	Methods
	-------
	kernel_matrix(new_data)
		Evaluates the rational quadratic kernel function at new_data.

	kernel_single_eval(new_data)
		Evaluates the rational quadratic kernel function at new_data involving double `for` loops;
	...
	def kernel_matrix(self, new_data):
		...
		-------
		numpy.ndarray
			The array of rational quadratic kernel function evaluations.
	...
	def kernel_single_eval(self, new_data):
		...
		-------
		numpy.ndarray
			The array of XXXXX kernel function evaluations.
		...
		for i in range(self.N):
			for j in range(n):
				sqsum = np.sum((self.data[i] - new_data[j]) ** 2)
				output[i][j] = 1 - sqsum / (sqsum + self.bw ** 2)
...
if __name__ == '__main__':
	...
	kernel = RationalQuadratic(data1, 1.4)
	...
	kernel = RationalQuadratic(data2, 2.3)
