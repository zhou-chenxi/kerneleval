...
class Sigmoid:
	...
	a : float
		The coefficient in front of the dot product of data vectors; must be strictly positive.
	c : float
		The additive constant; must be non-negative.

	Methods
	-------
	kernel_matrix(new_data)
		Evaluates the sigmoid kernel function at new_data.

	kernel_single_eval(new_data)
		Evaluates the sigmoid kernel function at new_data involving double `for` loops;
		less efficient than self.kernel_matrix.

	"""
	
	def __init__(self, data, a, c):
		...
		if a <= 0.:
			raise ValueError(f'The coefficient, a, must be strictly positive.')
		else:
			self.a = a
	...
	def kernel_matrix(self, new_data):
		...
		-------
		numpy.ndarray
			The array of sigmoid kernel function evaluations.
		...
		sigmoid_part = np.arctan(self.a * (split_prod) + self.c)
	...
	def kernel_single_eval(self, new_data):
		...
		for i in range(self.N):
			for j in range(n):
				output[i][j] = np.arctan(self.a * np.sum(self.data[i] * new_data[j]) + self.c)
...
if __name__ == '__main__':
	...
	kernel = Sigmoid(data2, 0.23, 0.54)
