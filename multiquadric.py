...
class Multiquadric:
	...
	"""
	This is a class of evaluating the multiquadric kernel function.

	Attributes
	----------
	data : numpy.ndarray
		The array at which the multiquadric kernel function is to evaluated.
	N, d : int, int
		The shape of data array.
	bw : float
		The bandwidth parameter in the multiquadric kernel function; must be strictly positive.

	Methods
	-------
	kernel_matrix(new_data)
		Evaluates the multiquadric kernel function at new_data.

	kernel_single_eval(new_data)
		Evaluates the multiquadric kernel function at new_data involving double `for` loops;
	...
	def kernel_matrix(self, new_data):
		...
		-------
		numpy.ndarray
			The array of multiquadric kernel function evaluations.
	...
	def kernel_single_eval(self, new_data):
		...
		for i in range(self.N):
			for j in range(n):
				sq_norm = np.sum((self.data[i] - new_data[j]) ** 2)
				output[i][j] = np.sqrt(1. + sq_norm / self.bw ** 2)
