...
class Laplacian:
	...
	"""
	This is a class of evaluating the Laplacian kernel function.

	Attributes
	----------
	data : numpy.ndarray
		The array at which the Laplacian kernel function is to evaluated.
	N, d : int, int
		The shape of data array.
	bw : float
		The bandwidth parameter in the Laplacian kernel function; must be strictly positive.

	Methods
	-------
	kernel_matrix(new_data)
		Evaluates the Laplacian kernel function at new_data.

	kernel_single_eval(new_data)
		Evaluates the Laplacian kernel function at new_data involving double `for` loops;
	...
	def kernel_matrix(self, new_data):
		...
		-------
		numpy.ndarray
			The array of Laplacian kernel function evaluations.
	...
	def kernel_single_eval(self, new_data):
		...
		for i in range(self.N):
			for j in range(n):
				output[i][j] = np.exp(- np.sum(np.abs(self.data[i] - new_data[j])) / self.bw)
